...
/// An object store bound to a signed-in session. All reads/writes go through the
/// vault's crypt remote so data is encrypted client-side by rclone crypt.
#[derive(Debug, Clone)]
...
impl DriveStore {
    pub fn new(cfg: &AppConfig, encrypted: bool) -> Self {
        DriveStore { cfg: cfg.clone(), encrypted }
    }
    ...
    fn remote(&self, relative_path: &str) -> String {
        if self.encrypted {
            ...
            rclone::cipher_spec(&self.cfg, relative_path)
        }
    }

    pub async fn get(&self, path: &str) -> Result<Vec<u8>> {
        let path = validate_relative_path(path)?;
        rclone::run_rclone(&self.cfg, &["cat".to_string(), self.remote(&path)])
    }
    ...
    async fn put_tempfile(&self, path: &str, data: &[u8]) -> Result<()> {
        let dir = self.cfg.temp_dir.join("store");
    ...
    pub async fn list(&self, path: &str) -> Result<Vec<ObjectEntry>> {
        ...
        let output = rclone::run_rclone(&self.cfg, &args)?;
    ...
    pub async fn exists(&self, path: &str) -> Result<bool> {
        ...
        let out = crate::rclone::run_rclone(
            &self.cfg,
            &["lsjson".to_string(), "--stat".to_string(), self.remote(&path)],
        );
        match out {
            Ok(_) => Ok(true),
            Err(_) => Ok(false),
    ...
    pub async fn delete(&self, path: &str) -> Result<()> {
        let path = validate_relative_path(path)?;
        rclone::run_rclone(&self.cfg, &["purge".to_string(), self.remote(&path)]).map(|_| ())
    }

    pub async fn write_json<T: Serialize>(&self, path: &str, value: &T) -> Result<()> {
        let bytes = serde_json::to_vec(value).map_err(|e| VaultError::Meta(format!("encode: {e}")))?;
